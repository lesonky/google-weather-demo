  ...
  if (!data) {
    return (
      <div className="p-6 bg-white rounded-lg shadow-md">
        <p className="text-gray-500">请选择一个位置查看每日天气预报</p>
      </div>
    );
  }
        ...
        {data.days.map((day, index) => (
          <div key={index} className="py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-gray-500">{day.temperatureLow.value}°</span>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-3">
              <div className="flex flex-col">
                <span className="text-xs text-gray-500">日出</span>
                <span className="text-sm">{formatTime(day.sunrise)}</span>
              </div>
              
              <div className="flex flex-col">
                <span className="text-xs text-gray-500">日落</span>
                <span className="text-sm">{formatTime(day.sunset)}</span>
              </div>
              
              <div className="flex flex-col">
                <span className="text-xs text-gray-500">降水概率</span>
                <span className="text-sm">{day.precipitationProbability.value}%</span>
              </div>
              
              <div className="flex flex-col">
                <span className="text-xs text-gray-500">风速</span>
              ...
              <div className="flex flex-col">
                <span className="text-xs text-gray-500">紫外线</span>
                <span className="text-sm">{day.uvIndex.value}</span>
              </div>
              
              <div className="flex flex-col">
                <span className="text-xs text-gray-500">降水量</span>
